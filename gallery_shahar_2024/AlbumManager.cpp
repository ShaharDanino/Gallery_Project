...
void AlbumManager::closeProc()
{
	if (_handles.empty()) {
		std::cout << "no pictures to close\n";
		return;
	}

	HANDLE handle = _handles.front();

	TerminateProcess(handle, 0);
	CloseHandle(handle);
	if(!_handles.empty()) _handles.pop_front();
}
...
void AlbumManager::showPicture()
{
	...
	{
		std::cout << "What program do you want to use in opening the picture: \n 1. Paint\n 2. IrfanView\n 3. windows's photos\n";
		std::cin >> choice;
	}
	...
	{
	case 1:
		cmdLine = "mspaint \"" + pic.getPath() + "\"";
		break;
	case 2:
		cmdLine = "C:\\Program Files\\IrfanView\\i_view64.exe \"" + pic.getPath() + "\"";
		break;
	case 3:
	...
	if (choice != 3){
		...
		{
			throw std::runtime_error("Showing picture failed" + std::to_string(GetLastError()));
		}
		else
		{
			_handles.push_back(pi.hProcess);
		}
	}
}
...
void AlbumManager::userStatistics()
{
	...
	int ownedAlbums = m_dataAccess.countAlbumsOwnedOfUser(user);
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Number of albums owned by user: " << ownedAlbums << std::endl;
}
